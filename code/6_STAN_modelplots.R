...
formerge <- df %>% select(id, treatment, mc, mr)
background_points <- read.csv(here::here("data/cleaned/posteriors", "allometric_individualSTAN.csv")) %>% as_tibble() %>%
  pivot_wider(names_from = .variable, values_from = .value) %>%
  mutate(a = a/48, 
         h = h*48) %>%
...
p3 <- forplot %>% 
  filter(parameter == "h") %>%
  mutate(c.max = (1/mu), 
         c.max.lower = (1/mu.lower), 
         c.max.upper = (1/mu.upper)) %>%
  ggplot(aes(x = mc, y = c.max))+
  geom_point(data = background_points, aes(x = mc, y = c.max/mr, color = treatment), alpha = 0.5)+
  geom_ribbon(aes(ymin = c.max.lower, ymax = c.max.upper, y = c.max, x = mc, group = treatment), fill = "gray", alpha = 0.25)+
  geom_line(aes(x= mc, y = c.max, color = treatment, linetype = treatment), show.legend = T, lwd = 2)+
  scale_color_manual(values = urchin_cols)+
  scale_y_log10()+
  scale_x_log10()+
  labs(x = "Predator body mass (g)", y = "Maximum consumption rate (ind. / hour)", color = "", linetype = "")+
  cowplot::theme_cowplot()+
...
p4 <- forplot %>% 
  filter(parameter == "h") %>%
  mutate(c.max = (1/mu)*mr, 
         c.max.lower = (1/mu.lower)*mr, 
         c.max.upper = (1/mu.upper)*mr) %>%
  ggplot(aes(x = mc, y = c.max))+
  geom_point(data = background_points, aes(x = mc, y = c.max, color = treatment), alpha = 0.5)+
  geom_ribbon(aes(ymin = c.max.lower, ymax = c.max.upper, y = c.max, x = mc, group = treatment), fill = "gray", alpha = 0.25)+
  geom_line(aes(x= mc, y = c.max, color = treatment, linetype = treatment), show.legend = T, lwd = 2)+
  scale_color_manual(values = urchin_cols)+
  scale_y_log10()+
  scale_x_log10()+
  labs(x = "Predator body mass (g)", y = "Maximum consumption rate (g urchins / hour)", color = "", linetype = "")+
  cowplot::theme_cowplot()+
  theme(legend.position = c(0.6, 0.2))

out <- cowplot::plot_grid(p3, p4)
ggsave("figures/p4_forAS.png",out,  width = 10, height = 6)
